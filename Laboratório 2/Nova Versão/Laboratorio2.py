...
def calcula_ucb(nodo):  # Calcula o valor do critério UCB (Upper Confidence Bound), para um nodo da árvore.
    ...
    criterio_ucb = (nodo.vitorias / nodo.jogadas) + CONSTANTEUCB * math.sqrt(
        (2 * math.log(nodo.pai.jogadas)) / nodo.jogadas)
    return criterio_ucb
...
def simulacao(raiz):  # Realiza a etapa de SIMULAÇÃO do Monte Carlo Tree Search (MCTS).
    ...
    while True:  # Enquanto houver simulações possíveis, realiza os lances de forma aleatória (SIMULAÇÃO).
        lances_possiveis = acha_lances(nodo_aux.tabuleiro,
                                       nodo_aux.jogador)  # Calcula a lista com todos os lances possíveis, na posição atual.
...
def monte_carlo(raiz,jogador,oponente):
    ...
    while time.time()-tempo_inicio < LIMITETEMPO: # Enquanto não atingir o limite de tempo de execução, continua explorando o jogo.
        ...
        while (nodo_melhor.pai != None): # Etapa de RETROPROPAGAÇÃO do algoritmo Monte Carlo Tree Search (MCTS).
            nodo_melhor.jogadas += 1
            if vencedor == nodo_melhor.jogador: # Incrementa o número de vitórias, encorajando o BOT a seguir o caminho.
                nodo_melhor.vitorias += 1 # Atualiza número de vitórias do FILHO.
            else: # Decrementa o número de vitórias, desencorajando o BOT a seguir o caminho.
                nodo_melhor.vitorias -= 1 # Atualiza número de vitórias do FILHO.
